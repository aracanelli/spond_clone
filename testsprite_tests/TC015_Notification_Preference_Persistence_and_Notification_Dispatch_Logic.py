...
async def run_test():
    ...
    try:
        ...
        # -> Click 'Sign In' to log in as test-admin@spond-test.com to configure notification preferences.
        ...
        frame = context.pages[-1]
        # Input email for test-admin@spond-test.com login
        ...
        # -> Click 'Get Started' to begin configuring notification preferences.
        frame = context.pages[-1]
        # Click 'Get Started' button on notification preferences modal
        elem = frame.locator('xpath=html/body/div[5]/div[2]/div/div[3]/button[2]').nth(0)
        await page.wait_for_timeout(3000); await elem.click(timeout=5000)
        

        # -> Select a mobile carrier from the dropdown to enable SMS notifications, then click Continue.
        frame = context.pages[-1]
        ...
        # -> Select a carrier (e.g., Rogers) to enable SMS notifications and then click Continue.
        frame = context.pages[-1]
        # Select 'Rogers' carrier from the dropdown
        elem = frame.locator('xpath=html/body/div[7]/div/div/div[2]').nth(0)
        await page.wait_for_timeout(3000); await elem.click(timeout=5000)
        

        # -> Click Continue to save SMS notification preferences and proceed to next notification preference configuration.
        frame = context.pages[-1]
        # Click Continue button to save SMS notification preferences
        elem = frame.locator('xpath=html/body/div[5]/div[2]/div/div[3]/button[2]').nth(0)
        await page.wait_for_timeout(3000); await elem.click(timeout=5000)
        

        # -> Click 'Complete Setup' to save current notification preferences and proceed to dashboard.
        frame = context.pages[-1]
        ...
        try:
            ...
            raise AssertionError("Test plan failed: Notification delivery did not follow user preferences with correct priority (web push first if enabled, then email-to-SMS if enabled, and always email as fallback). Test case execution failed.")
