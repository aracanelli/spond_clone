...
async def run_test():
    ...
    try:
        ...
        # -> Click 'Sign In' to log in as organizer (test-organizer@spond-test.com).
        ...
        # -> Input organizer email test-organizer@spond-test.com and click Continue.
        frame = context.pages[-1]
        # Input organizer email for login
        ...
        # -> Input password for organizer and click Continue to log in.
        frame = context.pages[-1]
        # Input password for test-organizer@spond-test.com
        ...
        # -> Input admin email test-admin@spond-test.com and click Continue to proceed with login.
        frame = context.pages[-1]
        ...
        # -> Input password for test-admin@spond-test.com and click Continue to log in.
        frame = context.pages[-1]
        ...
        # -> Dismiss the welcome modal by clicking 'Skip for now' to access the dashboard fully.
        frame = context.pages[-1]
        ...
        # -> Click on the 'Soccer Practice' event to view event details and manage RSVP invitations.
        frame = context.pages[-1]
        # Click on 'Soccer Practice' event to open event details
        elem = frame.locator('xpath=html/body/div/main/div/div/div/div/div[2]/div/div[2]/div/a').nth(0)
        await page.wait_for_timeout(3000); await elem.click(timeout=5000)
        ...
        frame = context.pages[-1]
        try:
            await expect(frame.locator('text=Attendance Confirmed for Nonexistent User').first).to_be_visible(timeout=1000)
        except AssertionError:
            raise AssertionError("Test case failed: Attendance responses are not tracked accurately or cannot be viewed by event organizers as per the test plan.")
